...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("RUBAB~")) {
    console.log("❌ Invalid MUZAMIL-XD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ MUZAMIL-XD session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'MUZAMIL'}</p>
        </div>
      </body>
      </html>
    `);
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://ibb.co/ymzhC8LZ'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 MUZAMIL-XD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ MUZAMIL-XD Bot connected to WhatsApp!");
...
> _Message was deleted but muzamil and Rubab saved it_ 🔰`;
